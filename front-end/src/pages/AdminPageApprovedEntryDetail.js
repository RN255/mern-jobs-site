...
function EntryDetail() {
  ...
  const [showUnapproveModel, setShowUnapproveModal] = useState(false);

  const handleCloseUnapproveModal = () => setShowUnapproveModal(false);
  const handleOpenUnapproveModal = () => setShowUnapproveModal(true);
  ...
  const handleUnapprove = () => {
    axios
      .put(`/api/entries/entries/${id}`, { approved: false })
      .then((response) => {
    ...
    <div className="container entryDetailsPage marginForNavBar">
      {dataLoaded && isAuthenticated ? (
        <>
          ...
          <div className="row">
            <div className="col">
              <Button onClick={handleOpenUnapproveModal} className="me-1">
                Unapprove
              </Button>
              ...
              <Modal
                show={showUnapproveModel}
                onHide={handleCloseUnapproveModal}
                animation={false}
              >
                <Modal.Header closeButton>
                  <Modal.Title className="text-warning text-uppercase">
                    Mild Warning
                  </Modal.Title>
                </Modal.Header>
                <Modal.Body>
                  ...
                  this? It will be taken off the site and return to the
                  "unapproved jobs" list.
                </Modal.Body>
                <Modal.Footer className="d-flex justify-content-between">
                  <Button
                    variant="outline-success"
                    className="m-0"
                    onClick={handleCloseUnapproveModal}
                  >
                    No, do not unapprove
                  </Button>
                  <Button
